General:
	...
	vim.map("/","<GoTo_Find>","General")
	vim.map("K","<Alt_up>","General")
	vim.map("H","<Alt_left>","General")
	vim.map("L","<Alt_right>","General")
	;vim.map("t","<NewTab>","General")
	;vim.map("x","<CloseTab>","General")
	;vim.map("X","<CloseWindow>","General")
	;vim.map("K","<NextTab>","General")
	;vim.map("J","<PrevTab>","General")
	;vim.map("gg","<Go_Home>","General")
	;vim.map("G","<Go_End>","General")

	vim.comment("<Alt_up>","Alt_up")
	vim.comment("<Alt_left>","Alt_left")
	vim.comment("<Alt_right>","Alt_right")
	vim.comment("<NextTab>","下一个标签")
	vim.comment("<PrevTab>","前一个标签")
	vim.comment("<CloseTab>","关闭当前标签")
	vim.comment("<CloseWindow>","关闭当前窗口")
    ...
    vim.Comment("<GoTo_Find>","查询")
    vim.Comment("<Go_Home>","移动到开头")
    vim.Comment("<Go_End>","移动到结尾")
...
<GoTo_Find>:
	...
	OutputVar := StrReplace(path,"\" ,"\\")
	pathbeging := "^"
	patchend := "\\"
	ControlSetText, Edit1, %pathbeging%%OutputVar%%patchend%, A
	Send {End}
return
